...
### 整型变量定义
Move中的整型目前只有3种，分别是`u8`、`u64`和`u128`。Move不支持有符号整型（signed integers），从目前来看未来也并没有引入有符号整型的计划，但后续会引入其他字节长度的无符号整型。
...
a != b;  // true
```
关于等号相关的一些知识点，需要更深入的了解 Move 语言其他的一些特性后再来进行讲解。
...
## 地址
地址（Address） 是 Move 中的一种类型，用于表示全局存储中的位置(或者称为帐户)，地址是一个128bit的标志符。

尽管地址是一个128位的整型，但 Move 并不允许通过整型来创建地址，也不允许地址进行任何的数学运算，也不允许改变，总的来说 Move 不允许地址发生动态的变化。

你可以在运行时通过地址的值来访问对应地址上的资源（Resources，这部分后面的章节会讲到），但不能在运行时通过来访问地址上的 module（前面的章节应该有讲到，module 是需要发布到具体的地址上的）。
...
```
也就是说全局存储了两个 Map ，一个用来存每个地址有哪些 Resource，一个用来存每个地址有哪些 Module。通过运行时只能访问第一个 Map 中的数据，也就是存 Resource 的那个。
...
```
脚本做的事情就是，先给 **s** 一个 `Coin`，然后我们再确认一下是否给成功了。
